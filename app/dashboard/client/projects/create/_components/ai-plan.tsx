...
export function AiPlan({ idea, answers, mode, modelId, onNext }: AiPlanProps) {
  ...
  const handleObjectListItemChange = (field: string, index: number, key: string, value: string) => {
    const newList = [...plan[field]];
  ...
  const addItem = (field: string, defaultValue: any) => {
    updateField(field, [...plan[field], defaultValue]);
  };

  const removeItem = (field: string, index: number) => {
    updateField(field, plan[field].filter((_: any, i: number) => i !== index));
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto pb-10">
      <div className="flex items-center justify-between">
        <div className="text-left space-y-1">
          ...
          <p className="text-zinc-500 dark:text-zinc-400">Review and refine the comprehensive strategic plan.</p>
        </div>
        <Button 
            ...
            <CardContent className="space-y-8">
               <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {isEditing && (
                      <Button variant="ghost" size="sm" onClick={() => addItem("deliverables", "New Deliverable")}>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {plan.deliverables.map((item: string, i: number) => (
                      <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 border relative group">
                        ...
                        {isEditing ? (
                          <div className="flex-1 flex items-center gap-2">
                            <Input 
                              value={item} 
                              onChange={(e) => handleListItemChange("deliverables", i, e.target.value)}
                              className="h-8 py-0 text-sm border-none bg-transparent focus-visible:ring-1"
                            />
                            <Button variant="ghost" size="icon" className="h-6 w-6 text-zinc-400 hover:text-red-500" onClick={() => removeItem("deliverables", i)}>
               ...
               <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {isEditing && (
                      <Button variant="ghost" size="sm" onClick={() => addItem("milestones", { title: "New Milestone", description: "", duration: "1 week" })}>
                        <Plus className="h-3 w-3 mr-1" /> Add
                      </Button>
                    )}
                  </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            {isEditing ? (
                              <Input 
                                value={m.title} 
                                onChange={(e) => handleObjectListItemChange("milestones", i, "title", e.target.value)}
                            ...
                            <div className="flex items-center gap-2">
                              {isEditing ? (
                                <Input 
                                  value={m.duration} 
                                  onChange={(e) => handleObjectListItemChange("milestones", i, "duration", e.target.value)}
                              ...
                              {isEditing && (
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-zinc-400 hover:text-red-500" onClick={() => removeItem("milestones", i)}>
                                  <Trash2 className="h-3 w-3" />
                                </Button>
                              )}
                            </div>
                          ...
                          {isEditing ? (
                            <Textarea 
                              value={m.description} 
                              onChange={(e) => handleObjectListItemChange("milestones", i, "description", e.target.value)}
                ...
                <div className="p-6 rounded-2xl bg-zinc-950 text-zinc-50 space-y-6 animate-in fade-in slide-in-from-top-2 border shadow-2xl">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      {isEditing && (
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] text-zinc-400" onClick={() => addItem("technicalSpecs", { category: "Category", tech: "Tool", reason: "Reason" })}>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {plan.technicalSpecs.map((spec: any, i: number) => (
                      <div key={i} className="relative group space-y-2 p-3 rounded-lg border border-zinc-800 bg-zinc-900/50">
                        {isEditing && (
                          <Button 
                            variant="ghost" 
                            size="icon" 
                        ...
                        {isEditing ? (
                          <>
                            <Input 
                              value={spec.category} 
                              onChange={(e) => handleObjectListItemChange("technicalSpecs", i, "category", e.target.value)}
                              className="h-6 text-[9px] uppercase font-bold bg-transparent border-zinc-800"
                            />
                            <Input 
                              value={spec.tech} 
                              onChange={(e) => handleObjectListItemChange("technicalSpecs", i, "tech", e.target.value)}
                              className="h-7 text-sm font-bold bg-transparent border-zinc-800"
                            />
                            <Textarea 
                              value={spec.reason} 
                              onChange={(e) => handleObjectListItemChange("technicalSpecs", i, "reason", e.target.value)}
        ...
        <div className="space-y-6">
          <Card className="bg-zinc-50/50 dark:bg-zinc-900/50 border-none">
            <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
              ...
              {isEditing && (
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => addItem("risks", { risk: "New Risk", mitigation: "" })}>
                  ...
                  {isEditing ? (
                    <div className="space-y-2">
                       <div className="flex items-center gap-2">
                        <Input 
                          value={r.risk} 
                          onChange={(e) => handleObjectListItemChange("risks", i, "risk", e.target.value)}
                          className="h-7 text-xs font-bold"
                        />
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-zinc-400" onClick={() => removeItem("risks", i)}>
                          <Trash2 className="h-3 w-3" />
                        </Button>
                       </div>
                       <Textarea 
                        value={r.mitigation} 
                        onChange={(e) => handleObjectListItemChange("risks", i, "mitigation", e.target.value)}
          ...
          <Card className="bg-zinc-50/50 dark:bg-zinc-900/50 border-none">
            <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
              ...
              {isEditing && (
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => addItem("successMetrics", "New Metric")}>
                  <Plus className="h-3 w-3" />
                </Button>
              )}
            </CardHeader>
            <CardContent>
              <ul className="space-y-2">
                {plan.successMetrics.map((m: string, i: number) => (
                  <li key={i} className="group flex items-start gap-2 text-[11px] text-zinc-600 dark:text-zinc-400 leading-snug">
                    <span className="text-zinc-900 dark:text-white mt-1 shrink-0">•</span>
                    {isEditing ? (
                      <div className="flex-1 flex items-center gap-2">
                        <Input 
                          value={m} 
                          onChange={(e) => handleListItemChange("successMetrics", i, e.target.value)}
                          className="h-7 text-[10px] py-0"
                        />
                        <Button variant="ghost" size="icon" className="h-5 w-5 text-zinc-400" onClick={() => removeItem("successMetrics", i)}>
                          <Trash2 className="h-3 w-3" />
                        </Button>
                      </div>
                    ) : (
          ...
          <Card className="bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 shadow-xl overflow-hidden relative">
            ...
            <CardContent className="p-6 space-y-2">
              ...
              <p className="text-[10px] opacity-60 italic pt-2">Based on current analysis of scope complexity.</p>
            </CardContent>
          </Card>
          ...
          {isEditing && <p className="text-center text-[10px] text-zinc-500 italic">Exit edit mode to continue.</p>}
        </div>
      </div>
    </div>
  );
